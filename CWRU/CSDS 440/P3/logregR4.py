...
def logreg(path, useFullSample, iterationNums):
    '''
    This method is the main method to take in all options and compute the logistic regression model with adaboost
    ...
    if useFullSample == 0:
        ...
        for m in range(iterationNums):
            ...
            alpha = 0.5 * np.log((1-error)/(error))  
            trainingData[-1][m] = alpha
            minDW = min(data_weight)
            for i in range(len(data_weight)):
                trueLabel = 1 if data[i][-1] == 1 else -1
                data_weight[i] = data_weight[i] * np.exp(-alpha * trueLabel * classLabelArr[i]) + minDW * 0.01
            data_weight = data_weight / sum(data_weight)   
