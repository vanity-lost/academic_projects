...
def logreg(path, useFullSample, iterationNums, k):
    '''
    ...
    :param k: the value of k in alpha updates
    ...
    if useFullSample == 0:
        ...
        for m in range(iterationNums):
            ...
            for i in range(len(data)):
                ...
                if classLabelArr[i] != trueLabel:
                    error += data_weight[i]
                if classLabelArr[i] == trueLabel and trueLabel == 1:
                    TPWeight += data_weight[i]
            # print('error: ', error)
            
            if error == 0 or error >= 0.5:
                break
            alpha = 0.5 * np.log((1-error)/(error)) + k * np.exp(TPWeight)   # voting = 79, volcanoes = 80
            trainingData[-1][m] = alpha
            for i in range(len(data_weight)):
                trueLabel = 1 if data[i][-1] == 1 else -1
                data_weight[i] = data_weight[i] * np.exp(-alpha * trueLabel * classLabelArr[i])
