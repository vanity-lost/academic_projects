from parseData import create_dataset, create_dataset_skewed
import numpy as np
import os
...
def cal_relevance_v2(prior_knowledge, prior_knowledge_transfer):
    ...
    return positive_relevance, negative_relevance, (positive_relevance+negative_relevance)/2

def cal_model(postive_folder_path, negative_folder_path, training_size, testing_size):
    training_data_table, testing_data_table = create_dataset(postive_folder_path, negative_folder_path, training_size, testing_size)
    y1_count = 0
    y0_count = 0
...
def cal_model_skewed(postive_folder_path, negative_folder_path, training_size, testing_size):
    training_data_table, testing_data_table = create_dataset_skewed(postive_folder_path, negative_folder_path, training_size, testing_size)
    y1_count = 0
    y0_count = 0
    for i in range(len(training_data_table)-1):
        if training_data_table[i + 1][-1] == 1:
...
def NB_relevant_NB_retraining_v2_progress(postive_folder_path, negative_folder_path, training_size, testing_size, postive_folder_path_transfer, negative_folder_path_transfer, training_size_transfer, testing_size_transfer):
    # pre-trained model
    y1_prob, y0_prob, prior_knowledge, training_data_table, testing_data_table = cal_model(postive_folder_path, negative_folder_path, training_size, testing_size)
    # print('Pre-model is built')
...
def NB_relevant_NB_retraining_v3(postive_folder_path, negative_folder_path, training_size, testing_size, postive_folder_path_transfer, negative_folder_path_transfer, training_size_transfer, testing_size_transfer):
    # pre-trained model
    y1_prob, y0_prob, prior_knowledge, training_data_table, testing_data_table = cal_model(postive_folder_path, negative_folder_path, training_size, testing_size)
    print('Pre-model is built')
...
def NB_relevant_NB_retraining_v3_progress(postive_folder_path, negative_folder_path, training_size, testing_size, postive_folder_path_transfer, negative_folder_path_transfer, training_size_transfer, testing_size_transfer):
    # pre-trained model
    y1_prob, y0_prob, prior_knowledge, training_data_table, testing_data_table = cal_model(postive_folder_path, negative_folder_path, training_size, testing_size)
    print('Pre-model is built')
    
    # the current small model
    y1_prob_transfer, y0_prob_transfer, prior_knowledge_transfer, training_data_table_transfer, testing_data_table_transfer = cal_model_skewed(postive_folder_path_transfer, negative_folder_path_transfer, training_size_transfer, testing_size_transfer)
    print('Target-model is built')
...
def cal_model_v2(postive_folder_path, negative_folder_path, training_size, testing_size):
    training_data_table, testing_data_table = create_dataset(postive_folder_path, negative_folder_path, training_size, testing_size)
    y1_count = 0
    y0_count = 0
    for i in range(len(training_data_table)-1):
    ...
    for i in range(len(training_data_table[0])):
        ...
        if count11+count01 == 0 or count11+count10 == 0 or count10+count00 == 0 or count01+count00 == 0:
            chi2 = 1000000
        else:
            chi2 = count_sum * (count11 * count00 - count01 * count10) ** 2 / ((count11+count01)*(count11+count10)*(count10+count00)*(count01+count00))
    ...
    return y1_prob, y0_prob, prior_knowledge, training_data_table, testing_data_table
def cal_model_v3(postive_folder_path, negative_folder_path, training_size, testing_size):
    training_data_table, testing_data_table = create_dataset(postive_folder_path, negative_folder_path, training_size, testing_size)
    y1_count = 0
    y0_count = 0
    for i in range(len(training_data_table)-1):
    ...
    for i in range(int(len(score_list))):
        if score_list[i][1] > 10.83:
            del prior_knowledge[score_list[i][0]]
    return y1_prob, y0_prob, prior_knowledge, training_data_table, testing_data_table
def cal_model_v4(postive_folder_path, negative_folder_path, training_size, testing_size):
    training_data_table, testing_data_table = create_dataset(postive_folder_path, negative_folder_path, training_size, testing_size)
    y1_count = 0
    y0_count = 0
    for i in range(len(training_data_table)-1):
    ...
    # print(score_list[0:100])
    for i in range(int(len(score_list))):
        if score_list[i][1] < 3.841:
            del prior_knowledge[score_list[i][0]]
    return y1_prob, y0_prob, prior_knowledge, training_data_table, testing_data_table

def NB_relevant_NB_retraining_v3_analysis(postive_folder_path, negative_folder_path, training_size, testing_size, postive_folder_path_transfer, negative_folder_path_transfer, training_size_transfer, testing_size_transfer):
    # pre-trained model
    y1_prob, y0_prob, prior_knowledge, training_data_table, testing_data_table = cal_model_v2(postive_folder_path, negative_folder_path, training_size, testing_size)
    print('Pre-model is built')
    ...
    print('accuracy_relevant_transfer: ', accuracy_relevant_transfer)
    return accuracy_original, accuracy_transfer, accuracy_relevant_transfer
def NB_relevant_NB_retraining_v3_analysis_v2(postive_folder_path, negative_folder_path, training_size, testing_size, postive_folder_path_transfer, negative_folder_path_transfer, training_size_transfer, testing_size_transfer):
    # pre-trained model
    y1_prob, y0_prob, prior_knowledge, training_data_table, testing_data_table = cal_model_v3(postive_folder_path, negative_folder_path, training_size, testing_size)
    print('Pre-model is built')
    ...
    print('accuracy_relevant_transfer: ', accuracy_relevant_transfer)
    return accuracy_original, accuracy_transfer, accuracy_relevant_transfer
def NB_relevant_NB_retraining_v3_analysis_v3(postive_folder_path, negative_folder_path, training_size, testing_size, postive_folder_path_transfer, negative_folder_path_transfer, training_size_transfer, testing_size_transfer):
    # pre-trained model
    y1_prob, y0_prob, prior_knowledge, training_data_table, testing_data_table = cal_model_v4(postive_folder_path, negative_folder_path, training_size, testing_size)
    print('Pre-model is built')
...
def NB_relevant_NB_retraining_v5(postive_folder_path, negative_folder_path, training_size, testing_size, postive_folder_path_transfer, negative_folder_path_transfer, training_size_transfer, testing_size_transfer):
    # pre-trained model
    y1_prob, y0_prob, prior_knowledge, training_data_table, testing_data_table = cal_model(postive_folder_path, negative_folder_path, training_size, testing_size)
    print('Pre-model is built')
...
def NB_relevant_NB_retraining_v5_progress(postive_folder_path, negative_folder_path, training_size, testing_size, postive_folder_path_transfer, negative_folder_path_transfer, training_size_transfer, testing_size_transfer):
    # pre-trained model
    y1_prob, y0_prob, prior_knowledge, training_data_table, testing_data_table = cal_model(postive_folder_path, negative_folder_path, training_size, testing_size)
    print('Pre-model is built')
    ...
    for word in prior_knowledge.keys():
        ...
        prob_rank.append([probX_Y1, word])
    prob_rank.sort(key = lambda x:x[0], reverse=True)
    positive_table = [pair[1] for pair in prob_rank[:int(len(prob_rank)/3)]]
    negative_table = [pair[1] for pair in prob_rank[int(2*len(prob_rank)/3):]]
    ...
    for word in prior_knowledge.keys():
        ...
        if word in positive_table or word in negative_table:
            if word in prior_knowledge_transfer.keys():
                ...
                negative_learning_rate = 0.05 if positive_chi2 < negative_chi2 else 0.01
                overall_chi2 = max([positive_chi2, negative_chi2])
                while abs(prior_overall_chi2 - overall_chi2) >= 0.00001: 
                    probX_Y1 += (probX_Y1_transfer - probX_Y1) * positive_learning_rate
                    probX_Y0 += (probX_Y0_transfer - probX_Y0) * negative_learning_rate
                    prior_knowledge[word] = [probX_Y1, probX_Y0]
                    positive_chi2, negative_chi2 = cal_chi2(probX_Y1, probX_Y0, probX_Y1_transfer, probX_Y0_transfer)
...
def NB_relevant_NB_retraining_v4(postive_folder_path, negative_folder_path, training_size, testing_size, postive_folder_path_transfer, negative_folder_path_transfer, training_size_transfer, testing_size_transfer):
    # pre-trained model
    y1_prob, y0_prob, prior_knowledge, training_data_table, testing_data_table = cal_model(postive_folder_path, negative_folder_path, training_size, testing_size)
    print('Pre-model is built')
    ...
    for t in range(10):
        for word in prior_knowledge.keys():
            [probX_Y1, probX_Y0] = prior_knowledge[word]
            predict_prob[word] = [probX_Y1 * y1_prob_transfer, probX_Y0 * y0_prob_transfer]
        positive_relevance, negative_relevance, overall_relevance = cal_relevance_v2(prior_knowledge, prior_knowledge_transfer)
        positive_relevance2, negative_relevance2, overall_relevance2 = cal_relevance_v2(prior_knowledge, prior_knowledge)
        y1_prob_transfer = overall_relevance * positive_relevance + overall_relevance2 * positive_relevance2
        y0_prob_transfer = overall_relevance * negative_relevance + overall_relevance2 * negative_relevance2
        for word in prior_knowledge.keys():
            [predictY1_X, predictY0_X] = predict_prob[word]
            probX_Y1 = overall_relevance * positive_relevance * predictY1_X + overall_relevance2 * positive_relevance2 * predictY1_X
            probX_Y0 = overall_relevance * negative_relevance * predictY1_X + overall_relevance2 * negative_relevance2 * predictY0_X
            prior_knowledge[word] = [probX_Y1, probX_Y0]
    print('Retraining end...')
