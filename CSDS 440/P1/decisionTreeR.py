...
def entropy_label(data):
    ...
    entropy = - np.log2(fraction0 * fraction1)  if fraction0 * fraction1 != 0 else 0
    return entropy
...
def entropy_attribute(data, attributeIndex, continuous):
    ...
    if (continuous):
        ...
        for label in attributeLabel:
            ...
            entropy1 = size1/len(data) * len(bin11)/size1 * len(bin01)/size1 if size1 != 0 else 0

            entropy0 = 0.0
            size0 = len(bin10) + len(bin00)
            entropy0 = size0/len(data) * len(bin10)/size0 * len(bin00)/size0 if size0 != 0 else 0
        ...
        for attributeLabel in attributeLabels:
            ...
            entropy1 = 0.0
            size1 = len(bin11) + len(bin01)
            entropy1 = size1/len(data) * len(bin11)/size1 * len(bin01)/size1
            entropy += entropy1
