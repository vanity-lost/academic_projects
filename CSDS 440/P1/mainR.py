from mldata import *
import random
from decisionTreeR import *
from decisionTreeR import Root_Node
from decisionTreeR import decisionTree
import sys
...
#iterative deepening for the normal decision model to automatically find the best depth with the highest accuracy
def iterativeDeepening(path, crossValidation, gainRatio):
    lastAccuracy = 0
    lastSize = 0
    lastDepth = 0
    lastFirstFeature = 0
    ...
    while end:
        accuracy, size, depth, firstFeature = buildModel(path, crossValidation, currentDepth, gainRatio)
        if accuracy <= lastAccuracy:
            return lastAccuracy, lastSize, lastDepth, lastFirstFeature
        else:
            lastAccuracy = accuracy
            lastSize = size
            lastDepth = depth
            lastFirstFeature = firstFeature
            currentDepth += 1
    return None,None,None,None

#iterative deepening for the research decision model to automatically find the best depth with the highest accuracy
def iterativeDeepeningR(path, crossValidation, gainRatio):
    lastAccuracy = 0
    lastSize = 0
    lastDepth = 0
    lastFirstFeature = 0
    ...
    while end:
        accuracy, size, depth, firstFeature = buildModelR(path, crossValidation, currentDepth, gainRatio)
        if accuracy <= lastAccuracy:
            return lastAccuracy, lastSize, lastDepth, lastFirstFeature
        else:
            lastAccuracy = accuracy
...
def main(**Option):
    ...
    if maxDepthLimit != -1:
        ...
        accuracy, size, maxDepth, firstFeature = iterativeDeepening(path, useFullSample, useGainRatio)
...
#build the research decision tree model and return the evaluation of the model based on the input
#path specifies the name of the file
#crossValidation should be inpt as an int, 0 indicates cross validation, 1 indicates full sample
#maxDepthLimit should be input as an int, 0 is no depth limit, any other positive integer is the model's max depth
#gainRatio should be input as an int, 0 is information gain, 1 is gain ratio
def buildModelR(path, crossValidation, maxDepthLimit, gainRatio):
    import os
    rootdir, file_base = os.path.split(path)
    rootdir = rootdir[1:] if rootdir[1:] != '' else '.'
    # convert to list of list here
    ...
    if crossValidation == 0:
        ...
        for i in range(len(folds)):
            trainingSet = list()
            testingSet = list()
            attributeData = retrieveAttributeTypeR(file_base, rootdir)
            for j in range(len(folds)):
                if i != j:
                    for example in folds[j]:
                        trainingSet.append(example)
...
def retrieveAttributeTypeR(file_base, rootdir='.'):
    attributeList = list()
...
def getAccuracyAndTime(path, crossValidation, maxDepthLimit, gainRatio):
    ...
    for i in range(5):
        if maxDepthLimit != -1:
            ...
            accuracy, size, maxDepth, firstFeature = iterativeDeepening(path, crossValidation, gainRatio)
            SumOfAccuracy += accuracy
            SumOfSize += size
    SumOftime = time.time() - start_time
    return SumOftime/5, SumOfAccuracy/5, SumOfSize/5

#run the model 5 times to get the average size and accuracy of a given model
def getAccuracyAndSize(path, crossValidation, maxDepthLimit, gainRatio, useBinary):
    SumOfAccuracy = 0.0
    SumOfSize = 0
    for i in range(5):
        if maxDepthLimit != -1:
            if useBinary == 0:
                ...
                accuracy, size, maxDepth, firstFeature = buildModelR(path, crossValidation, maxDepthLimit, gainRatio)
                SumOfAccuracy += accuracy
                SumOfSize += size
        else:
            if useBinary == 0:
                ...
                accuracy, size, maxDepth, firstFeature = iterativeDeepeningR(path, crossValidation, gainRatio)
                SumOfAccuracy += accuracy
                SumOfSize += size
    return SumOfAccuracy/5, SumOfSize/5
