...
def Logregression(path, useFullSample, lambdaConstant):
    '''
    ...
    if useFullSample == 0:
        ...
        return np.mean(accuracyList), calSD(accuracyList), np.mean(precisionList), calSD(precisionList), np.mean(recallList), calSD(recallList), areaUnderROC, FPRs, TPRs
    else:
        weights, b = buildModel(data, attributeData, lambdaConstant)
        accuracy, precision, recall = evalution(data, weights, b)
        areaUnderROC, FPRs, TPRs  = calcAUC(classLabels)
        return accuracy, 0, precision, 0, recall, 0, areaUnderROC, FPRs, TPRs
