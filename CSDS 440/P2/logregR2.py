...
def updateWeights(attributeValues, y, weights, b, learningRate, length, lambdaConstant, count_itr, lrType):
    '''
    ...
    :param count_itr: the number of iterations
    :param lrType: the type of learning rate
    ...
    if lrType == 1:
        ...
        learningRate *= (1. / (1. + 0.5 * count_itr))
    elif lrType == 3:
        learningRate = np.exp(- 0.1 * count_itr)
    # grad desc for equation
    return weights, b, min(dw), db, learningRate


def buildModel(trainingSet, attributeData, lambdaConstant, lrType, learningRate = 1, converge_changeW = 0.0001, converge_changeB = 0.01):
    '''
    ...
    while abs(changeForw) >= converge_changeW or abs(changeForb) >= converge_changeB:
        weights, b, changeForw, changeForb, learningRate = updateWeights(attributes, classy, weights, b, learningRate, len(trainingSet), lambdaConstant, count_itr, lrType)
        count_itr += 1
        if learningRate <= 0:
            break
    return weights, b
...
def Logregression(path, useFullSample, lambdaConstant, lrType):
    '''
    ...
    :param lrType: the type of learning rate convergence used
    ...
    if useFullSample == 0:
        ...
        for i in range(len(folds)):
            ...
            weights, b = buildModel(trainingSet, attributeData, lambdaConstant, lrType)
        ...
        return np.mean(accuracyList), calSD(accuracyList), np.mean(precisionList), calSD(precisionList), np.mean(recallList), calSD(recallList), areaUnderROC, SumOftime
    else:
        weights, b = buildModel(data, attributeData, lambdaConstant, lrType)
        accuracy, precision, recall = evalution(data, weights, b)
        areaUnderROC  = calcAUC(classLabels)
        SumOftime = time.time() - start_time
        return accuracy, 0, precision, 0, recall, 0, areaUnderROC, SumOftime
...
def main(**Option):
    ...
    import time
    start_time = time.time()
    accuracy, accuracySTD, precision, precisionSTD, recall, recallSTD, areaUnderROC, FPRs, TPRs = Logregression(path, useFullSample, lambdaConstant)
    SumOftime = time.time() - start_time
    print(SumOftime)
