...
def updateWeights(attributeValues, y, weights, b, learningRate, length, lambdaConstant, data_weights):
    '''
    This method updates the weight of each attribute using gradient descent, but with data weight now
...
def buildModel(trainingSet, attributeData, data_weights, lambdaConstant = 0.5, learningRate = 0.05, converge_changeW = 0.05, converge_changeB = 1):
    ...
    :param lambdaConstant: lambda from the penalty term,  use 0.5 as the base line
    :param learningRate: the learning rate, use 0.5 as the base line
    :param converge_changeW: the converge difference for w, use 0.05 as the base line, but will change by len(trainingSet)
    :param converge_changeB: the converge difference for b, use 1 as the base line, but will change by len(trainingSet)
...
def evalution(trainingData, testingSet):
    ...
    :param trainingData: first row is all classifier,  e.g. we have trainingData[0][m] = [weight, b]
...
def logreg(path, useFullSample, iterationNums):
    '''
    This method is the main method to take in all options and compute the logistic regression model with AD adaboost
    ...
    if useFullSample == 0:
        ...
        data_weight = removeNoise(data, trainingData)
        
        trainingData = list()
        for i in range(2):
            row = [0] * iterationNums
            trainingData.append(row)
        accuracyDataList = list()
        for m in range(iterationNums):
            # print('Boosting... %1d / %1d' % (m+1, iterationNums))
            weights, b, data_weight = buildModel(data, attributeData, data_weight)
            trainingData[0][m] = [weights, b]
            ...
            accuracyDataList.append(accuracy)
        if len(accuracyDataList) < iterationNums:
            for j in range(iterationNums-len(accuracyDataList)):
                accuracyDataList.append(accuracy)
        return accuracyDataList

def removeNoise(data, trainingData):
    m = []
    ml = 0
    for i in range(len(data)):
        trueLabel = data[i][-1]
        ...
        for t in range(len(trainingData[-1])):
            if trainingData[0][t] == 0:
                break
            prediValue = trainingData[0][t][1]
            for p in range(len(trainingData[0][t][0])):
                prediValue += trainingData[0][t][0][p] * data[i][p]
            classLabel = 1 if prediValue > 0 else -1
            mi += trainingData[-1][t] * classLabel
        m.append(mi * trueLabel)
